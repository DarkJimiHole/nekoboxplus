...
import moe.matsuri.nb4a.po0.Po0Whitelist
...
class BaseService {
    ...
    class Data internal constructor(private val service: Interface) {
        ...
        var po0Whitelist: Po0Whitelist? = null

        val receiver = broadcastReceiver { ctx, intent ->
            when (intent.action) {
                Intent.ACTION_SHUTDOWN -> service.persistStats()
                Action.RELOAD -> service.reload()
                Action.PO0_WHITELIST_REFRESH -> po0Whitelist?.requestRefresh()
    ...
    interface Interface {
        ...
        fun killProcesses() {
            data.po0Whitelist?.close()
            data.po0Whitelist = null
            data.proxy?.close()
            wakeLock?.apply {
                release()
                wakeLock = null
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            if (!data.closeReceiverRegistered) {
                val filter = IntentFilter().apply {
                    ...
                    addAction(Action.PO0_WHITELIST_REFRESH)
            ...
            data.connectingJob = runOnMainDispatcher {
                try {
                    ...
                    preInit()
                    data.po0Whitelist = Po0Whitelist(this@Interface)
                    data.po0Whitelist!!.start()
                    proxy.init()
