...
fun Project.setupApp() {
    ...
    android.apply {
        defaultConfig {
            ...
            resValue("string", "application_id", pkgName)
            versionCode = verCode
            versionName = verName
    ...
    android.apply {
        ...
        buildTypes {
            getByName("debug") {
                resValue("string", "application_id", "${pkgName}debug")
            }
            getByName("release") {
                proguardFiles(
            ...
            outputs.all {
                ...
                outputFileName = if (isPreview) {
                    outputFileName.replace(
                        project.name,
                        "NekoBoxPlus-" + requireMetadata().getProperty("PRE_VERSION_NAME")
                    ).replace("-preview", "")
                } else {
                    outputFileName.replace(project.name, "NekoBoxPlus-$versionName")
                        .replace("-release", "")
                        .replace("-oss", "")
                }
            }
